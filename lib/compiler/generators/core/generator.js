...
var Generator = function (options) {
	...
	this.buffers = [];
	this.buffer = null;
	this.nullBuffer = this.createNullBuffer();
...
Generator.prototype.onCommentToken = function (token) {
	if (this.currentTemplate != null) {
		this.currentTemplate.onCommentToken(token);
	}
	else {
...
Generator.prototype.appendComment = function (token, opt_buffer, opt_blockStyle) {
	var buffer = opt_buffer || this.buffer;
	// don't using token.text because it can be long and contain anything
	var text = 'comment';
	...
	buffer.push('/* ', text);
	this.appendLineNumber(token.ctx, opt_buffer, true);
	buffer.push(' */');
	if (opt_blockStyle) {
		buffer.push('\n\n');
	}
...
Generator.prototype.appendLineNumber = function (ctx, opt_buffer, opt_withinComment) {
	if (ctx != null) {
		var buffer = opt_buffer || this.buffer;
		if (!opt_withinComment) {
			buffer.push(' //');
		}
...
Generator.prototype.createNullBuffer = function () {
	return {
		push: function () {}
	};
};
