...
var Func = function (template, token) {
	...
	this.isCommented = this.token.isCommented;
...
Func.prototype.start = function () {
	...
	if (this.isCommented) {
		var buffer = this.obtainBuffer();
		this.appendCommented(buffer);
		this.buffer = this.generator.nullBuffer;
	}
	else {
...
Func.prototype.onCommentToken = function (token) {
	this.generator.appendComment(token, this.buffer);
};
...
Func.prototype.appendCommented = function (buffer, opt_entityName) {
	var entityName = opt_entityName || 'func';
	buffer.push(
		'// commented ', entityName, ': ', this.name
	);

	if (this.options.appendLineNumbersFor.func) {
		this.generator.appendLineNumber(this.token.ctx, buffer, true);
	}
};
