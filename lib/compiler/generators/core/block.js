...
Block.prototype.getBlockFunctionName = function (name) {
	return this.options.blockFuncNamePrefix + strTools.capFirst(name);
};
...
Block.prototype.onCommentToken = function (token) {
	this.appendComment(token);
};

Block.prototype.onBlockCallToken = function (token, opt_withDefinition) {
	this.appendBlockCall(token, opt_withDefinition);
};
...
Block.prototype.appendCommented = function (buffer) {
	if (this.template.blockStack.length > 1) {
		buffer.push('\n\n');
	}

	Block.super_.prototype.appendCommented.call(this, buffer, 'block');
};
...
Block.prototype.appendComment = function (token) {
	this.sequence.close(token.ctx);
	this.generator.appendComment(token, this.buffer, true);
};

Block.prototype.appendBlockCall = function (token, opt_withDefinition) {
	this.sequence.close(token.ctx);
	...
	if (token.isCommented) {
		this.buffer.push(
			...
			'// commented block',
			!!opt_withDefinition ? '' : ' call',
			': ', token.name
		);

		if (this.options.appendLineNumbersFor.blockCall) {
			this.generator.appendLineNumber(token.ctx, this.buffer, true);
		}
	}
	else {
		this.buffer.push(
...
Block.prototype.appendInsertTemplate = function (token) {
	...
	if (token.setupFunc) {
		this.buffer.push('.setup(function () {');
		if (this.options.funcLocals) {
			this.buffer.push(' ', this.options.funcLocals);
		}
		this.buffer.push(token.setupFunc,'})');
	}
	this.buffer.push(
