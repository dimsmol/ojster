...
var subpathRegExp = identifierRegExp+'(\\.'+identifierRegExp+')*';
var fullNameRegExp = identifierRegExp+'(\\.'+identifierRegExp+')*';

Tokenizer.prototype.requireCommandRegExp = RegExp('^\\s*((('+aliasRegExp+')(\\s*=\\s*(('+pathRegExp+')(\\.('+subpathRegExp+'))?))?(\\s+('+fullNameRegExp+'))?)|('+fullNameRegExp+'))\\s*$');
var aliasAndFullNameRegExp = RegExp('^\\s*((('+aliasRegExp+')(\\s+('+fullNameRegExp+'))?)|('+fullNameRegExp+'))\\s*$');
Tokenizer.prototype.templateCommandRegExp = aliasAndFullNameRegExp;
Tokenizer.prototype.inheritsCommandRegExp = aliasAndFullNameRegExp;
...
Tokenizer.prototype.parseRequireCommand = function(command) {
    ...
    if (match) {
        ...
        var fullName = match[14] || match[16];
		if (!fullName && !subpath && !path && alias) {
			fullName = alias; // cannot distinguish fullName from alias in this situation
		}
        return new tokens.Require(this.getCtx(), alias, path, subpath, fullName);
    } else {
...
Tokenizer.prototype.parseTemplateCommand = function(command) {
    ...
    if (match) {
		var alias = match[3];
		var fullName = match[5] || match[7] || alias; // cannot distinguish fullName from alias if only alias set
...
Tokenizer.prototype.parseInheritsCommand = function(command) {
    ...
    if (match) {
		var alias = match[3];
		var fullName = match[5] || match[7] || alias; // cannot distinguish fullName from alias if only alias set
        return new tokens.Inherits(this.getCtx(), alias, fullName);
    } else {
