...
Tokenizer.prototype.parseCommand = function(command) {
	...
	switch (name) {
		...
		case 'insert':
			return this.parseInsertTemplateCommand(rest);
		case 'require':
		...
		case 'set':
			return this.parseSetCommand(rest);
		case 'css':
			return this.parseCssCommand(rest);
		case 'space':
			return new tokens.Space(this.getCtx());
		default:
...
Tokenizer.prototype.insertCommandRegExp = RegExp('^\\s*('+fullNameRegExp+')(\\s*\\((.*)?\\))?\\s*$');

Tokenizer.prototype.parseInsertTemplateCommand = function(command) {
	match = command.match(this.insertCommandRegExp);
	if (match) {
		var templateName = match[1];
		var args = match[4];
		return new tokens.InsertTemplate(this.getCtx(), templateName, args);
	} else {
		this.error(new tokenizationErrors.InvalidInsertTemplateCommand(this.getCtx()));
		return tokens.InvalidToken;
	}
};
...
Tokenizer.prototype.parseCssCommand = function(command) {
	var match = command.match(/^\s*((\+)?(\S+)(\s+(\S+))?)?\s*$/);
	if (match) {
		var isPlus = (match[2] == '+');
		var arg1 = match[3];
		var arg2 = match[5];
		...
		if (isPlus) {
			if (arg2 != null) {
				this.error(new tokenizationErrors.InvalidCssCommandExtraArguments(this.getCtx()));
				return tokens.InvalidToken;
			}
			modifiers = arg1;
		} else {
		...
		return new tokens.Css(this.getCtx(), nameExpr, nameStr, modifiers);
	} else {
		this.error(new tokenizationErrors.InvalidCssCommand(this.getCtx()));
		return tokens.InvalidToken;
	}
};

Tokenizer.prototype.parseSetCommand = function(command) {
	match = command.match(/^\s*(\{.*\})\s*$/);
	if (match) {
		var props = match[1];
		return new tokens.Set(this.getCtx(), props);
	} else {
		this.error(new tokenizationErrors.InvalidSetCommand(this.getCtx()));
		return tokens.InvalidToken;
	}
};
