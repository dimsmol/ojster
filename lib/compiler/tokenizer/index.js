...
var Tokenizer = function (src, options) {
	...
	this.startCommentMark = marks.comment.start;
	this.endCommentMark = marks.comment.end;
...
Tokenizer.prototype.getOffset = function (str) {
	var tabCount = strTools.charCount(str, '\t');
...
Tokenizer.prototype.getInstruction = function () {
	var start = this.pos;
	var startWithin = this.pos + this.startInstructionMarkLength;
	var endMark = this.endInstructionMark;
	if (strTools.startsWith(this.src, this.startCommentMark, startWithin)) {
		startWithin += this.startCommentMark.length;
		// if got long comment, find appropriate end
		endMark = this.endCommentMark + this.endInstructionMark;
	}
	var end = this.src.indexOf(endMark, startWithin);

	if (end == -1) {
		throw new errors.CloseInstructionMarkMissed(this.getCtx());
	}
...
Tokenizer.prototype.parseInstruction = function (instruction) {
	...
	if (strTools.startsWith(instruction, this.commandMark, start)) {
		return this.parseCommand(instruction.substring(start + this.commandMarkLength, end));
	}
	else if (strTools.startsWith(instruction, this.startCommentMark, start)) {
		return new tokens.Comment(this.getCtx(), instruction.substring(start + this.startCommentMark.length, end - this.endCommentMark.length));
	}
	else {
