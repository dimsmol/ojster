...
var startsWith = function (str, startStr, pos) {
	var result = false;
	if (pos == null) {
		pos = 0;
	}
	if (startStr.length < str.length + pos) {
		result = (
			pos === 0 && startStr.length == str.length && startStr == str ||
			startStr == str.substr(pos, startStr.length)
		);
	}
	return result;
};
