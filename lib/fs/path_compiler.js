...
var compile = function (srcPath, dstPath, options, callback) {
	...
	var compileFile = function (filePath, dstFilePath, callback) {
		...
		fileCompiler.compile(filePath, dstFilePath, options, function (err) {
			if (err != null) {
				if (mt.is(err, CompilationFailed)) {
	...
	var q = async.queue(function (task, done) {
		...
		if (task.processPath) {
			currentPath = joinPath(srcPath, task.relativePath);
				...
				if (err) {
					...
					for (var i=0, l=items.length; i < l; i++) {
						if (items[i][0] != '.') { // skipping dot-files
							q.push({
								getStat: true,
								relativePath: joinPath(task.relativePath, items[i]),
				...
				if (err) {
					...
					if (stat.isDirectory()) {
						...
						if (dstPath != null) {
							...
							var dirPath = joinPath(dstPath, task.relativePath);
						...
						if (dstPath != null) {
							if (task.relativePath == null) {
								...
								dstFilePath = fileCompiler.getDefaultDstPath(currentPath, joinPath(dstPath, task.previousRelativePath), options.ext);
							}
						}
						q.push({
							compile: true,
